...
const MIN_SAMPLES_FOR_RECOMMENDATIONS = 60;
...
function Prediction() {
  ...
  const stopTest = async () => {
    ...
    const avgReadings = {
      ...
      temperature: ranges.temperature.avg,
        ...
        {status !== "done" ? (
          ...
          <div className="form-container report-panel">
            ...
            <div className="verdict-row">
              <span className={"tag risk-" + (report.riskLevel || "low").toLowerCase()}>
                Risk: {report.riskLevel}
              </span>
              <span className={"tag status-" + (report.status || "healthy").toLowerCase()}>
                Status: {report.status}
              </span>
              <span className="tag issues-tag">
                Issues: {report.issues && report.issues.length > 0 ? report.issues.join(", ") : "None"}
              </span>
            </div>
            ...
            <div className="report-actions">
              ...
              <Link to="/reports" className="btn">View All Reports</Link>
            </div>
          </div>
        )}
      </div>
